...
class Chat extends Api
{
    ...
    public function shake()
    {
		...
		if($this->request->isPost()){
			...
			if ($this->auth->isLogin()) {
			    ...
			    // 查询是否有离线消息
				$list = model('app\api1\model\wanlshop\Chat')
				...
				foreach($list as $message){
					...
					model('app\api1\model\wanlshop\Chat')->save(['online' => 1], ['id' => $message['id']]);
				}
		...
		foreach($query as $row)
		{
			if($row['type'] == 'chat'){ //临时
				$shop = model('app\api1\model\wanlshop\Shop')
					...
					->find();
				// 统计未读
				$count = model('app\api1\model\wanlshop\Chat')
	...
	public function send()
	{
		...
		if($this->request->isPost()){
			...
			// 保存聊天记录到服务器
			$data = model('app\api1\model\wanlshop\Chat');
	...
	public function history()
	{
		...
		if($this->request->isPost()){
			...
			$uid = $this->auth->id;
			// 查询历史记录
			$result = model('app\api1\model\wanlshop\Chat')
			...
			if($post['to_id'] == 0){
				if($post['message']['type'] == 'text'){
					if($content == '人工客服' || $content == '客服' || $content == '人工'){
						// 查询 哪个后台在线
						$online = [];
						$admin = model('app\api1\model\wanlshop\Admin')
						...
						$list = model('app\api1\model\wanlshop\Article')
						...
						if($list){
							...
							$article = model('app\api1\model\wanlshop\Article')
