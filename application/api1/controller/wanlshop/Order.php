...

{
...

    {
		...
		$list = model('app\api1\model\wanlshop\Order')
			...
			->each(function($order, $key){
				...
				$goods = model('app\api1\model\wanlshop\OrderGoods')
...

	{
		...
		if($this->request->isPost()){
			...
			foreach ($list as $row) {
			    ...
			    $row['goods'] = model('app\api1\model\wanlshop\OrderGoods')
...

    {
		...
		switch ($order['state']) {
			...
			default: // 获取物流
				...
				$eData = model('app\api1\model\wanlshop\KuaidiSub')
		...
		$order['address'] = model('app\api1\model\wanlshop\OrderAddress')
		...
		$order['goods'] = model('app\api1\model\wanlshop\OrderGoods')
	    ...
	    if ($this->request->isPost()) {
			...
			if($refund){
				...
				foreach($refund as $value){
					...
					controller('addons\wanlshop\library\WanlPay\WanlPay')->money(-$value['price'], $order['shop']['user_id'], 'there is a refund for this order', 'pay', $order['order_no']);
...

	{
	    ...
	    if ($this->request->isPost()) {
			...
			$row = model('app\api1\model\wanlshop\Order')->get(['id' => $id, 'user_id' => $this->auth->id]);
			...
			if($row['coupon_id'] != 0){
				...
				model('app\api1\model\wanlshop\CouponReceive')->where(['id' => $row['coupon_id'], 'user_id' => $this->auth->id])->update(['state' => 1]);

			}

			// 释放库存 1.0.3升级

			foreach(model('app\api1\model\wanlshop\OrderGoods')->all(['order_id' => $row['id']]) as $vo){

				// 查询商品是否需要释放库存

				if(model('app\api1\model\wanlshop\Goods')->get($vo['goods_id'])['stock'] == 'porder'){

					model('app\api1\model\wanlshop\GoodsSku')->where('id', $vo['goods_sku_id'])->setInc('stock', $vo['number']);
...

	{
	    ...
	    if ($this->request->isPost()) {
			...
			if($order['isaddress'] == 1){
				...
				$address = model('app\api1\model\wanlshop\Address')
				...
				$data = model('app\api1\model\wanlshop\OrderAddress')->save([
				...
				model('app\api1\model\wanlshop\Order')->where(['id' => $order_id, 'user_id' => $this->auth->id])->update(['isaddress' => 1]);
...

	{
	    ...
	    if ($this->request->isPost()) {
			...
			foreach ($post['goodsList'] as $value) {
				...
				model('app\api1\model\wanlshop\Goods')->where(['id' => $value['goods_id']])->setInc('comment');

				if($value['state'] == 0){

					model('app\api1\model\wanlshop\Goods')->where(['id' => $value['goods_id']])->setInc('praise');

				}else if($value['state'] == 1){

					model('app\api1\model\wanlshop\Goods')->where(['id' => $value['goods_id']])->setInc('moderate');

				}else if($value['state'] == 2){

					model('app\api1\model\wanlshop\Goods')->where(['id' => $value['goods_id']])->setInc('negative');

				}

			}

			if(model('app\api1\model\wanlshop\GoodsComment')->saveAll($commentData)){
			...
			$score = model('app\api1\model\wanlshop\GoodsComment')
			...
			model('app\api1\model\wanlshop\Shop')
	    ...
	    if ($this->request->isPost()) {
			...
			switch ($order['state']) {
				...
				default: // 获取物流
					...
					$express = model('app\api1\model\wanlshop\KuaidiSub')
...

    {
        ...
        if ($this->request->isPost()) {
		    ...
		    foreach ($request as $post) {
				...
				$sku = model('app\api1\model\wanlshop\GoodsSku')
			...
			foreach ($order as $key => $value) {
				...
				$config = model('app\api1\model\wanlshop\ShopConfig')
...

    {
        ...
        if ($this->request->isPost()) {
			...
			foreach ($lists as $item) {
				...
				$coupon = model('app\api1\model\wanlshop\CouponReceive')
				...
				$order = new \app\api1\model\wanlshop\Order;
				...
				if($order->save()){
					...
					foreach ($item['products'] as $data){
						...
						$goodsData = model('app\api1\model\wanlshop\Goods')->get($data['goods_id']);

						// 获取sku

						$sku = model('app\api1\model\wanlshop\GoodsSku')->get($data['sku_id']);
					...
					if($coupon){
						...
						if($priceAll >= $coupon['limit']){
							...
							model('app\api1\model\wanlshop\Coupon')
							...
							model('app\api1\model\wanlshop\Order')->destroy($order->id);
			...
			try {
			    ...
			    model('app\api1\model\wanlshop\OrderAddress')->saveAll($addressList);

			    model('app\api1\model\wanlshop\OrderGoods')->saveAll($goodsList);

			    $result = model('app\api1\model\wanlshop\Pay')->saveAll($payList);
			...
			if ($result !== false) {
				...
				model('app\api1\model\wanlshop\Order')->where(['id' => $order->id])->delete();
...

	{
		...
		$data = model('app\api1\model\wanlshop\ShopFreight')->where('id', $id)->field('id,delivery,isdelivery,name,valuation')->find();
		...
		if($data['isdelivery'] == 0){
			...
			$list = model('app\api1\model\wanlshop\ShopFreightData')
