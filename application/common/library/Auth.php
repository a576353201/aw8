...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'mobile', 'avatar', 'score','session'];
     ...
     * @return boolean
     */
    public function init($token,$session='')
    {
        if ($this->_logined) {
            return true;
        }
        ...
        if($session){
            ...
            $tokendata = Db::name('user')->where('session', $session)->column('id');
            //var_dump($tokendata);exit;
            if($tokendata){
                $expire = 86400 * 30 * 12;
                Cookie::set('uid', $tokendata[0], $expire);
                //Token::clear($tokendata[0]);
                $this->_token = Random::uuid();
                Token::set($this->_token, $tokendata[0], $this->keeptime);
                Cookie::set('token',$this->_token, $expire);
                $token = $session;
                $url = '/index/wanlshop/entry.html';
    ...
    public function direct($user_id)
    {
        ...
        if ($user) {
            ...
            try {
                ...
                $user->session = md5($time.$ip.$user_id);
                //记录本次登录的IP和时间
                $user->loginip = $ip;
                $user->logintime = $time;
                //重置登录失败次数
