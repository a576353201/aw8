...
class Signin extends Api
{
	...
	public function getSignin()
	{
		...
		if(!$config){
			$this->error('签到插件不存在！请安裝官方发布签到插件');
		}
	    ...
	    if ($this->request->isPost()) {
	        ...
	        if ($signin) {
	            ...
	            try {
	                ...
	                Db::rollback();
	                $this->error('签到失敗,请稍后重試');
	            }
	            $this->success('OK','签到成功!连续签到' . $successions . '天!獲得' . $score . '积分');
	        }
	    }
	    $this->error("请求錯誤");
	}
	...
	public function fillup()
	{
	    ...
	    if ($config['fillupdays'] < $days) {
	        $this->error("只允許補签{$config['fillupdays']}天的签到");
	    }
	    ...
	    if ($config['fillupnumsinmonth'] <= $count) {
	        $this->error("每月只允許補签{$config['fillupnumsinmonth']}次");
	    }
	    ...
	    Db::startTrans();
	    try {
	        \app\common\model\User::score(-$config['fillupscore'], $this->auth->id, '签到補签');
	        //尋找日期之后的
	        ...
	        Db::rollback();
	        $this->error('補签失敗,请稍后重試');
	    } catch (Exception $e) {
	        Db::rollback();
	        $this->error('補签失敗,请稍后重試');
	    }
	    $this->success('OK','補签成功');
	}
