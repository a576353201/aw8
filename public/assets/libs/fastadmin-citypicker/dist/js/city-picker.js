...
(function (factory) {
    ...
    CityPicker.prototype = {
        ...
        render: function () {
            var p = this.getPosition(),
                ...
                dropdown = '<div class="city-picker-dropdown" style="left:0px;top:100%;' +
                    ...
                    (this.includeDem('district') ? '<a data-count="district">區縣</a>' : '') + '</div>' +
        ...
        simplize: function (address, type) {
            address = address || '';
            if (type === PROVINCE) {
                return address.replace(/[省,市,自治區,壯族,回族,維吾爾,特別行政區]/g, '');
            } else if (type === CITY) {
                return address.replace(/[市,地區,回族,蒙古,苗族,白族,傣族,景頗族,藏族,彜族,壯族,傈僳族,布依族,侗族]/g, '')
                    .replace('哈薩克', '').replace('自治州', '').replace(/自治縣/, '');
            } else if (type === DISTRICT) {
                return address.length > 2 ? address.replace(/[市,區,縣,旗]/g, '') : address;
            }
        },
    ...
    CityPicker.DEFAULTS = {
        simple: false,
        responsive: true,
        render: 'text',//填充为代码還是文本，可选code或text
        placeholder: '请选择省/市/區',
        level: 'district',
        province: '',
        city: '',
        district: ''
