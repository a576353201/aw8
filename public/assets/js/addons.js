	...
	require(['chat'], function(Vue){
		...
		var wanlchat =  new Vue({
			...
			data:{
				...
				chatWindow: false, // 是否开啟聊天窗口

				isList: true, // 展示列表或按鈕 --

				isMsg: false, // 是否开啟消息彈窗 -- 
				...
				chatSelect: null, // 选中的条

				chatContent: [], //消息內容&历史条
			...
			methods: {
					...
					Fast.api.ajax({
						...
						ws.onmessage = (msg)=> {
							...
							if (data.type == 'init') {
								...
								// 更新主列表和 wanlchat列表，如果存在+1，如果不存在追加壹个列表
				...
				// 刪除窗口，如果只有壹个則直接关闭 判斷刪除的是否當前的，如果是讀取第壹个，如果不是直接刪掉即可
				...
				closeChat(){
					...
					this.chatWindow = false; // 是否开啟聊天窗口

					this.wanlchat =  []; // 聊天窗口列表

					this.chatSelect =  null; // 选中的条

					this.chatContent = []; //消息內容&历史条
				...
				miniChat(){
					...
					this.chatMiniWindow = !this.chatMiniWindow; // 是否开啟聊天窗口
				...
				onAudio(){
					...
					this.isAudio ? layer.msg('提示音已开啟', {icon: 1}):layer.msg('提示音已关闭', {icon: 2});
				...
				toggleBox() {
					...
					this.showBox = !this.showBox;  //通过控制showBox來控制box的显示与隐藏
				...
				latest(){
					...
					if(this.chatWindow){
						...
						this.$nextTick(() => {
							...
							let msg = document.getElementById('talk') // 獲取对象

							msg.scrollTop = msg.scrollHeight // 滾动高度
				...
				timeFormat(timestamp = null, fmt = 'yyyy-mm-dd'){
					...
					// yyyy:mm:dd|yyyy:mm|yyyy年mm月dd日|yyyy年mm月dd日 hh時MM分等,可自定義组合
					...
					// 判斷用戶输入的時間戳是秒還是毫秒,壹般前端js獲取的時間戳是毫秒(13位),后端传过來的为秒(10位)
