    ...
    var Controller = {
        index: function () {
			...
			$(document).on("click", ".btn-editprice", function () {
				...
				if($(this).data('id')){
                    Backend.api.open('wanlshop/order/detail1/id/' + $(this).data('id'), __('修改价格'),{area:['1000px', '700px']});
				}else{
			...
			$(document).on("click", ".btn-wholesale1", function () {
			    //alert('wanlshop/order/wholesale1/id/' + $(this).data('id'));
				if($(this).data('id')){
                    Backend.api.open('wanlshop/order/wholesale1/id/' + $(this).data('id'), __('壹键批发'),{area:['1000px', '700px']});
				}else{
				    alert('wanlshop/order/wholesale1/id/' + $(this).data('id'));
				}
			});
			...
			$(document).on("click", ".kuaidisub[data-id]", function () {
			    ...
			    Backend.api.open('wanlshop/order/relative/id/' + $(this).data('id'), __('快遞查詢'),{area:['800px', '600px']});
			...
			$(document).on("click", ".btn-express", function () {
				...
				Layer.alert('订单' + JSON.stringify(Table.api.selectedids(table)) + '无法雲打印,因为API接口維護中...');
