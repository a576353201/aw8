                ...
                if (['mobile', 'email'].indexOf(type) > -1) {
                    ...
                    element.isValid(function (v) {
                        if (v) {
                            ...
                            Layer.msg("请确认已經输入了正确的" + text + "！");
                        }
                    });
                } else {
                    var data = {event: $(btn).data("event")};
                    Frontend.api.sendcaptcha(btn, type, data, function (data, ret) {
                        Layer.open({title: false, area: ["400px", "430px"], content: "<img src='" + data.image + "' width='400' height='400' /><div class='text-center panel-title'>掃壹掃关注公眾号獲取验证码</div>", type: 1});
                    });
                }
                return false;
            });
