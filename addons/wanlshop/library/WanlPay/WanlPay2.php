	...
	public function pay($order_id)
	{
		...
		if ($this->type == 'balance') {
			...
			try {
				...
				// 修改用戶金额
				if(count($order_no) == 1){
					$result = self::money(-$wholesale_price, $user['id'], "balance payment order", 'pay', implode(",",$order_no));
				}else{
					$result = self::money(-$wholesale_price, $user['id'], "balance consolidated payment order", 'pay', implode(",",$order_no));
				}
